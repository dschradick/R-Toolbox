...
#### Diagnostik mit ggfortify
m <- glm(vs ~ mpg, data = mtcars, family=binomial)
autoplot(m, which = 1:6, label.size = 3)


## Cut-off Visualisierung
m$vs <- mtcars$vs
m$prediction <- predict(m,newdata = data.frame(mpg=mtcars$mpg),type='response')
p <- ConfusionMatrixInfo( data = m, predict = "prediction",  actual = "vs", cutoff = .6 )
p$plot
p$data$type


## Cut-off Bestimmung unter Verwendung von Kosten
ggthemr_reset()
cost_fp <- 100
cost_fn <- 200
roc_info <- ROCInfo(data = m, predict = "prediction",  actual = "vs",
